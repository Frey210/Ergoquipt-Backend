...
def _recording_to_csv(recording: HrvBulkRecording, readings: List[HrvBulkReading]) -> str:
    output = io.StringIO()
...
def _recording_to_json(recording: HrvBulkRecording, readings: List[HrvBulkReading]) -> str:
    payload = {
        "recording": {
            "id": str(recording.id),
